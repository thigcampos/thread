...
public struct SHA1 {
    ...
    public static func hash(_ input: [UInt8]) -> [UInt8] {
        ...
        var h0 = UInt32(littleEndian: 0x6745_2301)
        var h1 = UInt32(littleEndian: 0xEFCD_AB89)
        var h2 = UInt32(littleEndian: 0x98BA_DCFE)
        var h3 = UInt32(littleEndian: 0x1032_5476)
        var h4 = UInt32(littleEndian: 0xC3D2_E1F0)
        ...
        withUnsafePointer(to: &mlBigEndian) {
            message.append(
                contentsOf: Array(
                    UnsafeBufferPointer<UInt8>(start: UnsafePointer(OpaquePointer($0)), count: 8)))
        }
        ...
        for chunkStart in 0..<message.count / 64 {
            ...
            for index in 0...15 {
                let value = chunk.withUnsafeBufferPointer({
                    UnsafePointer<UInt32>(OpaquePointer($0.baseAddress! + (index * 4))).pointee
                })
            ...
            for i in 0..<80 {
                ...
                let temp = (rotateLeft(a, 5) &+ f &+ e &+ k &+ words[i]) & 0xFFFF_FFFF
                e = d
                d = c
                c = rotateLeft(b, 30)
                b = a
                ...
                digest.append(
                    contentsOf: Array(
                        UnsafeBufferPointer<UInt8>(
                            start: UnsafePointer(OpaquePointer($0)), count: 4)))
            }
        }
