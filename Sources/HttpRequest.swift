...
public class HttpRequest {
    ...
    public struct MultiPart {
        ...
        private func valueFor(_ headerName: String, parameter: String) -> String? {
            return headers.reduce([String]()) {
                ...
                return headerValueParams.reduce(
                    combined,
                    { (results, token) -> [String] in
    ...
    public func parseMultiPartFormData() -> [MultiPart] {
        ...
        guard let contentType = contentTypeHeaderTokens.first, contentType == "multipart/form-data"
        else {
            return []
        }
        var boundary: String?
    ...
    private func nextMultiPart(
        _ generator: inout IndexingIterator<[UInt8]>, boundary: String, isFirst: Bool
    ) -> MultiPart? {
        if isFirst {
    ...
    private func nextMultiPartBody(_ generator: inout IndexingIterator<[UInt8]>, boundary: String)
        -> [UInt8]?
    {
        var body = [UInt8]()
