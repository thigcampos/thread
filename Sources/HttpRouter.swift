...
open class HttpRouter {
    ...
    public func route(_ method: String?, path: String) -> (
        ...
        return queue.sync {
            if let method = method {
                ...
                if let handler = findHandler(
                    &rootNode, params: &params, generator: &pathSegmentsGenerator)
                {
    ...
    private func findHandler(
        _ node: inout Node, params: inout [String: String],
        ...
        findHandler(
            &node, params: &params, pattern: pattern, matchedNodes: &matchedRoutes, index: 0,
    ...
    private func findHandler(
        _ node: inout Node, params: inout [String: String], pattern: [String],
        matchedNodes: inout [Node], index: Int, count: Int
    ) {
        ...
        if index < count, let pathToken = pattern[index].removingPercentEncoding {
            ...
            if let variableNode = variableNodes.first {
                ...
                params[variableNode.0] = pathToken
                findHandler(
                    &node.nodes[variableNode.0]!, params: &params, pattern: pattern,
                    matchedNodes: &matchedNodes, index: currentIndex, count: count)
            }

            if var node = node.nodes[pathToken] {
                findHandler(
                    &node, params: &params, pattern: pattern, matchedNodes: &matchedNodes,
            ...
            if let startStarNode = node.nodes["**"] {
                ...
                while currentIndex < count,
                    let pathToken = pattern[currentIndex].removingPercentEncoding
                {
                    currentIndex += 1
                    if startStarNodeKeys.contains(pathToken) {
                        findHandler(
                            &startStarNode.nodes[pathToken]!, params: &params, pattern: pattern,
                            matchedNodes: &matchedNodes, index: currentIndex, count: count)
                    }
                }
            }
